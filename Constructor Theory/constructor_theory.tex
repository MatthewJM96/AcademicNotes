...
\begin{itemize}
	...
	\item[\textbf{Attribute}] A realised value of a property of a substrate (for example, a specific distance between two objects). An attribute is defined as the set of states of a substrate which have that attribute (i.e. that if we measured the substrate with respect to the corresponding property, we would obtain this attribute).
	\item[\textbf{Variable}] A set of disjoint attributes (i.e. no state in which any pair of attributes are held in common). \textit{Note}: two substrates in combination may have variables (and so attributes) that are not present independently in the two substrates - such as distance between centre of masses.
	\item[\textbf{Substrate State}] A set of fixings of variables of the substrate in question.
	\item[\textbf{Sharp Variable}] A variable of which a specific attribute is held in a substrate's state. That is to say, if a substrate were measured with respect to a sharp variable, the attribute that the variable is said to be sharp in would be obtained.
...
\begin{itemize}
	...
	\item[\textbf{Attribute}] $x \in X$ is an attribute in the variable $X$, it is defined as the set of all states which would yield the same value represented by this attribute (e.g. all states where two objects are 2m away from one another). Lower-case Latin alphabet characters in the same font are other attributes, except where specified.
...
Distinguishability lets us define the negation of an attribute on a substrate. That is, we can talk of all the attributes that are distinguishably not that attribute. For a substrate $\textbf{\textsc{S}}$, we write this as:
\begin{equation}
	\bar{x} \ \hat{=} \ \set{ \, y \ ; \ \forall \ y \in \textsc{S} \in \textbf{\textsc{S}} \ , \ y \perp x \, }
\end{equation}.
...
Above we defined the idea of measurement of a variable where it is sharp - that is we ask if one of a set of orthogonal .
